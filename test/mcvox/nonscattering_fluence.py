...
exportsrc = os.path.join(USER_TMP_PATH, 'mcvox_nonscattering_fluence.h')
make_user_dirs()
...
materials = mc.mcmaterial.Materials([
    ...
    mc.mcmaterial.Material(n=1.0, mua=4.0, mus=0.0, pf=pf),
])
...
voxels = mc.mcgeometry.Voxels(
    xaxis=mc.mcgeometry.Axis(-0.5e-3, 0.5e-3, 1),
    yaxis=mc.mcgeometry.Axis(-0.5e-3, 0.5e-3, 1),
    zaxis=mc.mcgeometry.Axis( 0.0e-3, 1.0,    1),
)

nphotons = 100e6

thickness = voxels.zaxis.span[-1] - voxels.zaxis.span[0]
...
for method, name in ((meth.ar, 'Albedo Rejection'), (meth.aw, 'Albedo Weight'),
    ...
    analytical_transmittance = np.exp(-materials[1].mua*thickness)
    print('-'*40)
    print('Transmittance (MC, analytical): ({:.6f}, {:.6f})'.format(
        float(mc_transmittance), float(analytical_transmittance)))

    analytical_deposition = -np.diff(
        np.exp(-materials[1].mua*fluence_res.zaxis.edges))
