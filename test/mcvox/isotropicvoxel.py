...
exportsrc = os.path.join(USER_TMP_PATH, 'mcvox_isotropicvoxel.h')
make_user_dirs()
...
source = mc.mcsource.IsotropicVoxel(
    voxels.index((0.0, 0.0, 5.0e-3))
)
...
for position in ((0.0, -2.5e-3, 5.0e-3), (0.0, 0.0, 4.0e-3), (0.0, 2.5e-3, 3.0e-3)):
    mc_obj.source.voxel = mc_obj.voxels.index(position)
    print(mc_obj.source.voxel)
    results = mc_obj.run(nphotons, exportsrc=exportsrc, verbose=True, out=results)

_, fluence_res, detectors_res = results
