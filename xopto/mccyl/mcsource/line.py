...
from xopto.mccyl.mcutil import boundary
...
class Line(Source):
    ...
    def cl_type(mc: mcobject.McObject) -> cltypes.Structure:
        ...
        class ClLine(cltypes.Structure):
            ...
            direction_reflected: mc_point3f_t
                Source direction in the medium after reflected from the sample
                surface.
            reflectance: mc_fp_t
    ...
    def cl_implementation(mc: mcobject.McObject) -> str:
        ...
        return '\n'.join((
            ...
            '			mcsim, mcsim_position(mcsim), ',
            '			&direction, source->reflectance);',
            '	#endif',
            '',
    ...
    def cl_pack(self, mc: mcobject.McObject, target: cltypes.Structure = None) \
        ...
        intersection, normal = mc.layers.intersect(
            self._position, self._direction, entrance=True)
        if intersection is None:
            raise ValueError('The Line source does not intersect the sample!')

        costheta = np.dot(normal, self._direction)
        reflectance = boundary.reflectance(
            mc.layer(0).n, mc.layer(1).n, costheta)
        if reflectance >= 1.0:
            raise ValueError('The line source is fully reflected from the '
                             'sample surface!')

        # refract the beam
        dir = boundary.refract(
            self._direction, normal, mc.layer(0).n, mc.layer(1).n)

        # reflected beam
        dir_reflected = boundary.reflect(self._direction, normal)

        target.position.fromarray(intersection)
