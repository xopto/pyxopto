...
class Fluence(mcobject.McObject):
    ...
    def cl_implementation(mc: mcobject.McObject):
        return '\n'.join((
            ...
            '',
            '	index_x = mc_int((mcsim_position_x(mcsim) - fluence->top_left.x)*',
            '		fluence->inv_step.x);',
            '	index_y = mc_int((mcsim_position_y(mcsim) - fluence->top_left.y)*',
            '		fluence->inv_step.y);',
            '	index_z = mc_int((mcsim_position_z(mcsim) - fluence->top_left.z)*',
            '		fluence->inv_step.z);',
            '',
            ...
            '		dbg_print_point3(INDENT    "voxel address:", &index_xyz);',
    ...
    def fromdict(cls, data: dict) -> 'Fluence':
        ...
        '''
        fluence_type = data.pop('type')
        if fluence_type != cls.__name__:
            raise TypeError('Expected "{}" type bot got "{}"!'.format(
                cls.__name__, fluence_type))
        x_axis = Axis.fromdict(data.pop('xaxis'))
        y_axis = Axis.fromdict(data.pop('yaxis'))
        z_axis = Axis.fromdict(data.pop('zaxis'))

        return Fluence(x_axis, y_axis, z_axis, **data)
    ...
    def _get_k(self) -> int:
        return self._k
    k = property(_get_k, None, None, 'Fluence floating point to accumulator'
    ...
    def _get_data(self):
        k = 1.0/(self.nphotons*self.dx*self.dy*self.dz)
        return self._data*k
