...
class Fluence(mcobject.McObject):
    ...
    def cl_implementation(mc: mcobject.McObject):
        return '\n'.join((
            ...
            '',
            '#if MC_FLUENCE_MODE_RATE',
            'inline void mcsim_fluence_deposit_at(McSim *mcsim, mc_point3_t const *pos, mc_fp_t weight, mc_fp_t mua){',
            '#else',
            'inline void mcsim_fluence_deposit_at(McSim *mcsim, mc_point3_t const *pos, mc_fp_t weight){',
            '#endif',
            '	mc_int_t index_x, index_y, index_z;',
            '	mc_size_t index;',
            '	__mc_fluence_mem McFluence const *fluence = mcsim_fluence(mcsim);',
            '',
            '	index_x = mc_int((pos->x - fluence->top_left.x)*',
            '		fluence->inv_step.x);',
            '	index_y = mc_int((pos->y - fluence->top_left.y)*',
            '		fluence->inv_step.y);',
            '	index_z = mc_int((pos->z - fluence->top_left.z)*',
            '		fluence->inv_step.z);',
            '',
...
class FluenceRz(mcobject.McObject):
    ...
    def cl_implementation(mc: mcobject.McObject):
        return '\n'.join((
            ...
            '	mc_fp_t r = mc_sqrt(dx*dx + dy*dy);',
            '',
            '	mc_fp_t dz = mcsim_position_z(mcsim) - fluence->center.z;',
            '',
            '	index_r = mc_int(r*fluence->inv_dr);',
            '	index_z = mc_int((mcsim_position_z(mcsim) - fluence->center.z)*',
            '		fluence->inv_dz);',
            '',
