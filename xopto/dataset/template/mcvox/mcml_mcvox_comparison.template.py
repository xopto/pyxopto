...
materials = mc.mcmaterial.Materials([
    ...
    mc.mcmaterial.Material(
        n={{ layer.n }}, mua={{ layer.mua }}, mus={{ layer.mus }}, pf=mc.mcpf.Hg({{ layer.g }})),
    {%- endfor %}
])

{% if sample.layers|length <= 3 %}
voxels = mc.mcgeometry.Voxels(
    xaxis=mc.mcgeometry.Axis(-1.0, 1.0, 2000),
    yaxis=mc.mcgeometry.Axis(-1.0, 1.0, 2000),
    zaxis=mc.mcgeometry.Axis(0.0, {{ sample.layers[1].d }},
                             int(round({{ sample.layers[1].d }}/0.001))),
)
{%- else %}
voxels = mc.mcgeometry.Voxels(
    xaxis=mc.mcgeometry.Axis(-1.0, 1.0, 2000),
    yaxis=mc.mcgeometry.Axis(-1.0, 1.0, 2000),
    zaxis=mc.mcgeometry.Axis(0.0, {{ sample.layers[1].d + sample.layers[2].d }},
                             int(round({{ sample.layers[1].d + sample.layers[2].d }}/0.0001))),
)
{%- endif %}
...
{%- else %}
voxels.material[:] = 2
voxels.material[z <= {{ sample.layers[1].d }}] = 1
{%- endif %}
...
for g in g_values:
    ...
    for mua in mua_values:
        for musr in musr_values:
            ...
            mc_obj.materials[1].mua = mua
            mc_obj.materials[1].mus = musr/(1.0 - g)
            if pos == 1:
