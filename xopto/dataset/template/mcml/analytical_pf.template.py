...
layers = mc.mclayer.Layers([
    ...
    mc.mclayer.Layer(
        d={{ layer.d }}, n={% if loop.first %}{{source.n_above }}{% elif loop.last %}{{ source.n_bellow }}{% else %}{{ layer.n }}{% endif %}, mua={{ layer.mua }}, mus={{ layer.mus }}, pf=mc.mcpf.{{pf.type}}({{ render_args_kwargs(pf.default) }})),
    {%- endfor %}
])
...
mc_obj = mc.Mc(layers, source, detectors, surface=surface, cl_devices=cl_device)
mc_obj.rmax = {{ rmax }}
...
{% if pf.type == 'Hg' -%}
N = {{ pf.g | length }}*{{ sample.n_mua }}*{{ sample.n_musr  }}
{% elif pf.type == 'MHg' -%}
N = {{ pf.g | length }}*{{ pf.b | length }}*{{ sample.n_mua }}*{{ sample.n_musr }}
{% elif pf.type == 'Gk' -%}
N = {{ pf.g | length }}*{{ pf.a | length }}*{{ sample.n_mua }}*{{ sample.n_musr }}
{% elif pf.type == 'MPc' -%}
N = {{ pf.n | length }}*{{ pf.b | length }}*{{ sample.n_mua }}*{{ sample.n_musr }}
{%- endif %}
pos = 0
{%- if pf.type == 'Hg' %}
for g in {{ pf.g }}:
    ...
    storage_dir = os.path.join(
        ...
        '{{ pf.param_dir[0] }}'.format(g).replace('.', '_')
    )
    os.makedirs(storage_dir, exist_ok=True)
    pf_info = 'Hg({:.2f})'.format(g)
    pos = process_mua_musr(storage_dir, g1, pf_info, num_packets, pos,
                           overwrite)
{% elif pf.type == 'MHg' %}
for g in {{ pf.g }}:
    for b in {{ pf.b }}:
        ...
        storage_dir = os.path.join(
            storage_dir_common, detector_dir, '{{ pf.dir }}',
            '{{ pf.param_dir[0] }}'.format(g).replace('.', '_'),
            '{{ pf.param_dir[1] }}'.format(b).replace('.', '_')
        )
        os.makedirs(storage_dir, exist_ok=True)
        pf_info = 'MHg({:.2f}, {:.2f})'.format(g, b)
        pos = process_mua_musr(storage_dir, g1, pf_info, num_packets, pos,
                               overwrite)
{% elif pf.type == 'Gk' %}
for g in {{ pf.g }}:
    for a in {{ pf.a }}:
        ...
        storage_dir = os.path.join(
            storage_dir_common, detector_dir, '{{ pf.dir }}',
            '{{ pf.param_dir[0] }}'.format(g).replace('.', '_'),
            '{{ pf.param_dir[1] }}'.format(a).replace('.', '_')
        )
        os.makedirs(storage_dir, exist_ok=True)
        pf_info = 'Gk({:.2f}, {:.2f})'.format(g, a)
        pos = process_mua_musr(storage_dir, g1, pf_info, num_packets, pos,
                               overwrite)
{% elif pf.type == 'MPc' %}
for n in {{ pf.n }}:
    for b in {{ pf.b }}:
        ...
        storage_dir = os.path.join(
            storage_dir_common, detector_dir, '{{ pf.dir }}',
            '{{ pf.param_dir[0] }}'.format(n).replace('.', '_'),
            '{{ pf.param_dir[1] }}'.format(b).replace('.', '_')
        )
        os.makedirs(storage_dir, exist_ok=True)
        pf_info = 'Mpc({:.2f}, {:.2f})'.format(n, b)
        pos = process_mua_musr(storage_dir, g1, pf_info, num_packets, pos,
                               overwrite)
{% else %}
raise TypeError('Unsupported scattering phase function!')
{% endif %}
print()
