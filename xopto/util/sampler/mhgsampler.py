...
from xopto.pf.util import MHgMap, MHgPolygon, g2gamma, g2delta


class MHgSampler(PfSampler):
    def __init__(self,
                 gamma: Tuple[float, float] or None = None,
                 delta: Tuple[float, float] or None = None,
                 g: Tuple[float, float] or None = None):
        '''
        Uniformly samples the native parameters of the MHG scattering phase
        ...
        g_sampler = UniformSampler(0.0, 1.0)
        b_sampler = UniformSampler(0.0, 1.0)

        super().__init__(mcpf.MHg, (g_sampler, b_sampler))
    ...
    @classmethod
    def fromdict(cls, data: dict) -> 'MHgSampler':
        '''
        ...
        if T != cls.__name__:
            raise TypeError(
                'Expected data for MHgSampler but got {}!'.format(T))

        return cls(**data)

    def __str__(self):
        return 'MHgSampler(gamma={}, delta={}, g={})'.format(
            self._gamma, self._delta, self._g)


class MHgGammaDeltaSampler(PfSampler):
    def __init__(self,
                 gamma: Tuple[float, float] or None = None,
                 delta: Tuple[float, float] or None = None,
                 g: Tuple[float, float] or None = None):
        '''
        Uniformly samples the MHG scattering phase function in
        ...
        gamma: Tuple[float, float] or None
            ...
            the full range of the MHG scattering phase function is used.
        delta: Tuple[float, float] or None:
        ...
        g: Tuple[float, float] or None:
            ...
            of their anisotropy.
        '''
        self._map = MHgMap.fromfile()
        self._polygon = MHgPolygon.fromfile()
        ...
        super().__init__(mcpf.MHg, (gamma_sampler, delta_sampler))
    ...
    def __call__(self, freeze=0):
        ...
        while True:
            ...
            err = ((gamma - gamma_)**2 + (delta - delta_)**2)**0.5
            if err < 1e-5:
                break
    ...
    @classmethod
    def fromdict(cls, data: dict) -> 'MHgGammaDeltaSampler':
        '''
        Create a new instance from a dict.
        ...
        if T != cls.__name__:
            raise TypeError(
                'Expected data for MHgGammaDeltaSampler but got {}!'.format(T))

        return cls(**data)

    def __str__(self):
        return 'MHgGammaDeltaSampler(gamma={}, delta={}, g={})'.format(
            self._gamma, self._delta, self._g)
...
if __name__ == '__main__':
    ...
    N = 100
    for s in [MHgSampler(gamma=(0.8, 1.5), g=(0.0, 0.99)),
              MHgGammaDeltaSampler(gamma=(0.8, 1.5), g=(0, 0.99))]:
        delta, gamma = [], []
        for i in range(N):
            gs = s().pf().gs(3)
