...
class LinearProbe:
    def __init__(
            ...
            fiber_count: int = 6,
            fiber_core_d: float = 200e-6,
            fiber_cladding_d: float = 220e-6,
        ...
        surrounding_ri:  float or Callable[[float, float], float]
            ...
            temperature.
            Default implementation uses the refractive index of the medium
            (parameter ri_medium).
        fiber_core_ri: float or Callable[[float, float], float]
            ...
            is used by default.
        fiber_count: int
            Number of linearly spaced optical fibers in the probe.
        fiber_core_d: float
        ...
        fiber_spacing: float or None
            ...
            tightly packed.
        src_fiber: int
            Index of the source fiber. Defaults to the first fiber with index 0.
        ...
        if fiber_spacing is None:
            fiber_spacing = fiber_cladding_d
        ...
        if probe_cutout:
            cutout = (
                fiber_spacing*(fiber_count - 1) + fiber_cladding_d,
                fiber_cladding_d
            )
        else:
            cutout = None

        top_surface_layout = mc.mcsurface.LinearArray(
            fiber=multimode_fiber,
            n=fiber_count,
            spacing=fiber_spacing,
            cutout=cutout,
            cutoutn=epoxy_fill_ri_value,
            reflectivity=probe_reflectivity_value,
        ...
        top_detector = mc.mcdetector.LinearArray(
            fiber=multimode_fiber,
            n=fiber_count,
            spacing=fiber_spacing,
