...
DEFAULT_CONFIG = {
    ...
    'fiber_sds': np.linspace(0.0, 1.0e-3, 26),
    'trace_maxlen': 512,
...
def process_config(config: dict) -> dict:
    ...
    config['source'] = mc.mcsource.UniformFiber(
        ...
        position=(config['fiber_sds'][0]*0.5, 0.0, 0.0)
    )
    ...
    config['trace'] = mc.mctrace.Trace(
        ...
        filter=mc.mctrace.Filter(
            ...
            pz=(fiber_cosmin, 1.0),
            r=(0.0, fiber_cfg['dcore']*0.5, (-config['fiber_sds'][0]*0.5, 0.0))
        )
    )
...
def sv_fiber_transmittance(filename: str, config: dict = None,
                           cl_device: clinfo.cl.Device = None,
                           overwrite: bool = False, verbose: bool = False):
    '''
    Generates data for visualization of the sampling volume in transmittance
    configuration as a function of the lateral distance between the source
    and detector fiber.
    ...
    N = len(config['fiber_sds'])
    sv_data = []
    sv = config['sv']
    ref_filter = config['trace'].filter
    half_sds = 0.5*(config['fiber_sds'].max() + config['fiber_sds'].min())
    for index, sds in enumerate(config['fiber_sds']):
        if verbose:
            print('Processing fiber transmittance SDS {:.1f} um; {}/{}'.format(
                sds*1e6, index + 1, N))

        mc_obj.trace.filter = mc.mctrace.Filter(
            z=ref_filter.z,
            pz=ref_filter.pz,
            r=(ref_filter.r[0][0], ref_filter.r[0][1], (sds - half_sds, 0))
        )

        mc_obj.source.position = (half_sds - sds, 0, 0)
        num_packets = 0
        sv.clear()
...
if __name__ == '__main__':
    ...
    common.main(sv_fiber_transmittance, common.create_sv_animation_xz,
